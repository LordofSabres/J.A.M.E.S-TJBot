...
console.log('Hello! My name is James! This version allows me to talk to you!');
...
const CONFIDENCE_THRESHOLD = 0.5;
     ...
     const utterance = msg.toLowerCase();
        ...
        if (response.object.intents !== undefined) {
            if(utterance != '%hesitation' || utterance != '%HESITATION'){
         ...
         console.log(tone);
          ...
          if (emotionalTones.length > 0) {
            ...
            if (maxTone.score >= CONFIDENCE_THRESHOLD) {
                ...
                else if (intent.intent != 'sarcastic' && utterance != '%hesitation')
                {
                    shineForEmotion(maxTone.tone_id);
        ...
        //END OF EMOTIONAL TONE ANALYSIS
                // if we didn't speak a response yet, speak it now
        if (spoken === false && utterance != '%hesitation') {
            await tj.speak(response.description);
        }
                }
