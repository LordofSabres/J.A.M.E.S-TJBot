...
console.log('You have successfully run this on start up!');
tj.pulse('red');
tj.pulse('red');
tj.pulse('red');
tj.pulse('red');
...
while (true) {
    ...
    if (msg === 'green') {
        console.log('Shining Green!');
        tj.shine('green');
    }
    
    if (msg === 'blue') {
        console.log('Shining Blue!');
        tj.shine('blue');
    }
     ...
     const utterance = msg.toLowerCase();
        ...
        if (response.object.intents !== undefined) {
            ...
            case 'greetings':
                ...
                break;
            //cases are IBM intents word for word, not dialog nodes
            case 'sarcastic':
                await tj.speak(response.description);
